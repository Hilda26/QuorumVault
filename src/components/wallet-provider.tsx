...
import { readContract } from "@/lib/quorumvault";

type Roles = Record<string, string[]>;
type WalletContextValue = { address?: `0x${string}`; connected: boolean; connect: () => Promise<`0x${string}`>; disconnect: () => void; error?: string; profile?: Roles };
const WalletContext = createContext<WalletContextValue | undefined>(undefined);

export function WalletProvider({ children }: { children: React.ReactNode }) {
  const [address, setAddress] = useState<`0x${string}`>(); const [error, setError] = useState<string>(); const [profile, setProfile] = useState<Roles>();
  ...
  useEffect(() => { if (!address) return; void readContract<Roles>("get_wallet_roles", [address]).then(setProfile).catch((cause) => setError(cause instanceof Error ? cause.message : "Unable to load wallet roles.")); }, [address]);
